...
EmberEventControl TimerReadTempLuxSensorControl;
...
void emberAfMainInitCallback(void)
{
	...
	/************************** Cai dat phan cung *********************/
	ledinit(porta,0);			//Khoi tao chan pin lam GPIO dieu khien LED
...
//	ledon(portc,2);
	...
	emberEventControlSetInactive(TimerReadTempLuxSensorControl);
	...
	emberEventControlSetDelayMS(TimerReadTempLuxSensorControl,2000);
	emberEventControlSetDelayMS(TimerCheckNetworkStartupControl,3000);
...
void networkEventHandle(uint8_t networkResult)
{
	...
	switch(networkResult)
	{
		...
		case REPORT_STATE:
		{
			...
			SEND_ReportInfoToHc();				//Gui thong tin cua thiet bi toi ZC de phuc vu tao Icon tren App
//			emberEventControlSetDelayMS(testRFEventControl,2000);
			break;
		}
		default:
			break;
...
void TimerSendDataToCoordinatorHandler(void)
{
	...
	arr_data[0] = data_zone_1.byte_1_bh1750; arr_data[1] = data_zone_1.byte_2_bh1750;
	calcu_data_to_send(BH1750, SET, CMD_ID_BH1750, ZONE_NUM_1, 0,(uint8_t *)arr_data, 2, data_send);
	SEND_Data(0x0000, 7, 7, data_send);

	arr_data[0] = data_zone_1.byte_1_lm75; arr_data[1] = data_zone_1.byte_2_lm75;
	calcu_data_to_send(LM75, SET, CMD_ID_LM75, ZONE_NUM_1, 0,(uint8_t *)arr_data, 2, data_send);
	SEND_Data(0x0000, 8, 8, data_send);

	arr_data[0] = data_zone_1.data_led[0];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_1, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 1, 1, data_send);

	arr_data[0] = data_zone_1.data_led[1];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_2, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 2, 2, data_send);

	arr_data[0] = data_zone_1.data_led[2];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_3, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 3, 3, data_send);

	arr_data[0] = data_zone_1.data_led[3];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_4, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 4, 4, data_send);

	arr_data[0] = data_zone_1.data_led[4];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_5, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 5, 5, data_send);

	arr_data[0] = data_zone_1.data_led[5];
	calcu_data_to_send(LED, SET, table_convert_to_command_id(LED_NUM_6, ZONE_NUM_1), ZONE_NUM_1, 0,(uint8_t *)arr_data, 1, data_send);
	SEND_Data(0x0000, 6, 6, data_send);
	emberEventControlSetDelayMS(TimerSendDataToCoordinatorControl,5000);
}

void TimerReadTempLuxSensorHandler(void)
{
	double value_temp = 0;
	double value_lux = 0;
	emberEventControlSetInactive(TimerReadTempLuxSensorControl);
	value_temp = LM75_read_temp();
	value_lux = BH1750_read_light();
//	emberAfCorePrintln("\n\nValue Light BH1750: %d Lux  ---   Value Temp LM75: %d\n\n",(uint16_t)value_lux, (uint16_t)value_temp);
	convert_dec_to_hex((uint16_t)value_lux, (unsigned int*)&data_zone_1.byte_1_bh1750, (unsigned int*)&data_zone_1.byte_2_bh1750);
	convert_dec_to_hex((uint16_t)value_temp, (unsigned int*)&data_zone_1.byte_1_lm75, (unsigned int*)&data_zone_1.byte_2_lm75);
//	emberAfCorePrintln("\n\nByte_1 LM75: 0x%x  ---   Byte_2 LM75: 0x%x\n\n",(uint8_t)data_zone_1.byte_1_lm75, (uint8_t)data_zone_1.byte_2_lm75);
//	emberAfCorePrintln("\n\nByte_1 BH1750: 0x%x  ---   Byte_2 BH1750: 0x%x\n\n",(uint8_t)data_zone_1.byte_1_bh1750, (uint8_t)data_zone_1.byte_2_bh1750);
	emberEventControlSetDelayMS(TimerReadTempLuxSensorControl,2000);
}
...
//			emberAfCorePrintln("Raw Data LM75: %d  --  %d",byRxBuffer2[4], byRxBuffer2[5]);
			...
			emberAfCorePrintln("Data LM75: %d",real_data);
...
//			emberAfCorePrintln("Raw Data BH1750: %d  --  %d",byRxBuffer2[4], byRxBuffer2[5]);
			...
			emberAfCorePrintln("Data BH1750: %d",real_data);
...
//			printf("\nLED_1_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[0] == 0)
			{
				ledoff(porta,0);
			}
			else if(data_zone_1.data_led[0] == 1)
			{
				ledon(porta,0);
			}
			break;
...
//			printf("\nLED_2_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[1] == 0)
			{
				ledoff(portd,0);
			}
			else if(data_zone_1.data_led[1] == 1)
			{
				ledon(portd,0);
			}
			break;
...
//			printf("\nLED_3_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[2] == 0)
			{
				ledoff(portd,1);
			}
			else if(data_zone_1.data_led[2] == 1)
			{
				ledon(portd,1);
			}
			break;
...
//			printf("\nLED_4_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[3] == 0)
			{
				ledoff(portd,2);
			}
			else if(data_zone_1.data_led[3] == 1)
			{
				ledon(portd,2);
			}
			break;
...
//			printf("\nLED_5_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[4] == 0)
			{
				ledon(portc,1);
			}
			else if(data_zone_1.data_led[4] == 1)
			{
				ledoff(portc,1);
			}
			break;
...
//			printf("\nLED_6_ZONE_1 Need handler !\n");
			...
			if(data_zone_1.data_led[5] == 0)
			{
				ledon(portc,2);
			}
			else if(data_zone_1.data_led[5] == 1)
			{
				ledoff(portc,2);
			}
			break;
